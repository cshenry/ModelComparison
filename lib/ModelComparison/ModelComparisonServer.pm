package ModelComparison::ModelComparisonServer;
...
our %async_run_methods = (
        'compare_models_async' => 'ModelComparison.compare_models',
);

our %async_check_methods = (
        'compare_models_check' => 'ModelComparison.compare_models',
);
...
sub call_method {
    ...
    my $ctx = ModelComparison::ModelComparisonServerContext->new($self->{loggers}->{userlog},
...
{
    # Service ModelComparison requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for ModelComparison but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = ModelComparison::ModelComparisonServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service ModelComparison.");
    }
...
package ModelComparison::ModelComparisonServerContext;

use strict;

=head1 NAME

ModelComparison::ModelComparisonServerContext
...
package ModelComparison::ModelComparisonServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = ModelComparison::ModelComparisonServer->new(
